...
def _try_auto_send_review(db: Session, review: Review) -> bool:
    ...
    if review.automation_mode != "auto" or review.risk_level != "низкий":
        log_action(
            ...
            "review_action",
            (
                f"Автоответ для отзыва #{review.id} не выполнен. "
    ...
    if not review.draft_reply:
        log_action(
            db,
            "review_action",
            f"Автоответ для отзыва #{review.id} не выполнен: отсутствует draft_reply.",
        )
        return False
    ...
    if sent:
        ...
        log_action(
            db,
            "review_action",
            f"Для отзыва #{review.id} автоматически отправлен ответ.",
        )
        return True
    ...
    log_action(
        db,
        "review_action",
        f"Для отзыва #{review.id} попытка автоответа завершилась ошибкой.",
    )
    return False
...
def reviews_page(
    ...
    return templates.TemplateResponse(
        "reviews/list.html",
        {
            ...
            "category_options": ["позитив", "нейтральный", "негатив", "вопрос"],
            "status_options": ["new", "drafted", "одобрен", "отправлен", "ошибка", "archived"],
            "risk_level_options": ["low", "medium", "high"],
            "automation_mode_options": ["auto", "review_required", "manual_only"],
            "rating_options": ["1", "2", "3", "4", "5"],
            "processing_mode_options": ["только_черновики", "требуется_проверка", "авто_ответ"],
        },
    )
...
def import_reviews_csv(db: Session = Depends(get_db)):
    ...
    if not csv_path.exists():
        return RedirectResponse(url="/reviews/?message=Файл reviews.csv не найден в корне проекта", status_code=303)
            ...
            try:
                ...
                if review is None:
                    review = Review(
                        ...
                        processing_mode=(row.get("processing_mode") or "").strip() or "требуется_проверка",
    ...
    log_action(
        ...
        (
            "Импортирован reviews.csv из корня проекта. "
    ...
    return RedirectResponse(
        url=(
            "/reviews/?message="
            f"Импорт reviews.csv завершён. Обработано: {processed_count}, создано: {created_count}, "
...
def load_reviews_from_ozon_api(db: Session = Depends(get_db)):
    ...
    logger.info("Запущена ручная загрузка отзывов из Ozon API через маршрут /reviews/load.")
    stats = load_reviews_from_api(db)
    logger.info(
        "Загрузка отзывов через /reviews/load завершена. processed=%s created=%s updated=%s loaded=%s errors=%s",
        stats["processed"],
        stats["created"],
        stats["updated"],
        stats["loaded"],
...
def reanalyze_reviews(db: Session = Depends(get_db)):
    ...
    logger.info("Запущен ручной переанализ отзывов через маршрут /reviews/reanalyze.")
    stats = reanalyze_all_reviews(db)
    log_action(
        db,
        "reviews_reanalyzed",
        (
            "Переанализ отзывов завершен. "
            f"Processed: {stats['processed']}, updated: {stats['updated']}, errors: {stats['errors']}."
        ),
    )
    logger.info(
        "Переанализ отзывов завершен. processed=%s updated=%s errors=%s",
        stats["processed"],
        stats["updated"],
        stats["errors"],
    )
    return RedirectResponse(
        url=(
            "/reviews/?message="
            f"Переанализ завершен. Обработано: {stats['processed']}, обновлено: {stats['updated']}, ошибок: {stats['errors']}"
        ),
        status_code=303,
    )
...
def generate_replies_for_new_reviews(db: Session = Depends(get_db)):
    ...
    for review in reviews:
        ...
        log_action(
            db,
            "review_action",
            f"Для отзыва #{review.id} автоматически сгенерирован черновик ответа.",
        )
    ...
    log_action(
        ...
        f"Массовая генерация черновиков завершена. Сгенерировано ответов: {processed}.",
    )
    return RedirectResponse(
        url=f"/reviews/?message=Сгенерировано черновиков для новых отзывов: {processed}",
        status_code=303,
    )
...
def generate_review_reply(review_id: int, db: Session = Depends(get_db)):
    ...
    log_action(
        db,
        "review_action",
        f"Для отзыва #{review.id} создан черновик ответа.",
    )
    return RedirectResponse(
        url=f"/reviews/{review.id}?message=Черновик ответа успешно создан",
        status_code=303,
    )
    ...
    if not reply_text:
        ...
        log_action(
            db,
            "review_action",
            f"Не удалось отправить ответ для отзыва #{review.id}: текст ответа пустой.",
        )
        return RedirectResponse(
            url=f"/reviews/{review.id}?message={quote('Не удалось отправить: текст ответа пустой')}",
            status_code=303,
        )
    ...
    review.send_status = "успешно" if success else "ошибка"
    review.sent_at = datetime.utcnow() if success else None
    review.status = "отправлен" if success else "ошибка"
    review.updated_at = datetime.utcnow()
    db.add(review)
    db.commit()
    ...
    log_action(
        ...
        "review_action",
        (
            f"Ручная отправка ответа для отзыва #{review.id} завершена. "
            f"external_id={review.external_id}. "
            f"source_type={review.source_type}. "
            f"Успех: {success}. "
            f"Статус код: {status_code}. "
            f"Ответ Ozon: {response_summary}"
        ),
    )

    ui_message = "Ответ отправлен" if success else f"Не удалось отправить: {result.get('message') or 'ошибка Ozon'}"
    return RedirectResponse(
    ...
    log_action(
        db,
        "review_action",
        f"Для отзыва #{review.id} сгенерирован черновик ответа.",
    )
    return RedirectResponse(
        url=f"/reviews/{review.id}?message=Черновик ответа успешно сгенерирован",
        status_code=303,
    )
...
def approve_review_reply(review_id: int, db: Session = Depends(get_db)):
    ...
    if not review.draft_reply:
        return RedirectResponse(
            url=f"/reviews/{review.id}?message=Сначала нужно сгенерировать черновик ответа",
            status_code=303,
        )
    ...
    log_action(
        db,
        "review_action",
        f"Для отзыва #{review.id} черновик ответа одобрен и перенесен в final_reply.",
    )
    return RedirectResponse(
        url=f"/reviews/{review.id}?message=Ответ одобрен",
        status_code=303,
    )
...
def edit_review_reply(
    ...
    log_action(
        db,
        "review_action",
        f"Для отзыва #{review.id} сохранен отредактированный финальный ответ.",
    )
    return RedirectResponse(
        url=f"/reviews/{review.id}?message=Финальный ответ сохранен",
        status_code=303,
    )
    ...
    log_action(
        db,
        "review_action",
        f"Для отзыва #{review.id} отредактирован финальный ответ.",
    )
    return RedirectResponse(
        url=f"/reviews/{review.id}?message=Финальный ответ сохранен",
        status_code=303,
...
def send_review_reply(review_id: int, db: Session = Depends(get_db)):
    ...
    if not review.final_reply and not review.draft_reply:
        message = "Нельзя отправить пустой ответ"
    ...
    log_action(
        ...
        "review_action",
        (
            f"Тестовая отправка ответа для отзыва #{review.id} завершена. "
            f"Успех: {success}. "
            f"Статус код: {status_code}. "
            f"Ответ Ozon: {response_summary}"
        ),
    ...
    if not review.final_reply and not review.draft_reply:
        return RedirectResponse(
            url=f"/reviews/{review.id}?message=Нельзя отправить пустой ответ",
            status_code=303,
        )
    ...
    try:
        ...
        log_action(
            db,
            "review_action",
            f"Для отзыва #{review.id} произошла ошибка отправки ответа: {exc}",
        )
        return RedirectResponse(
            url=f"/reviews/{review.id}?message=Ошибка отправки ответа",
            status_code=303,
        )

    review.send_status = "успешно" if sent else "ошибка"
    review.sent_at = datetime.utcnow() if sent else None
    review.status = "отправлен" if sent else "ошибка"
    review.updated_at = datetime.utcnow()
    db.add(review)
    db.commit()
    ...
    log_action(
        ...
        "review_action",
        (
            f"Для отзыва #{review.id} выполнена отправка ответа. "
            f"Итоговый статус: {'отправлен' if sent else 'ошибка'}."
        ),
    )
    return RedirectResponse(
        url=(
            f"/reviews/{review.id}?message="
            + ("Ответ отправлен" if sent else "Ошибка отправки ответа")
        ),
        status_code=303,
    )
    ...
    if not review.final_reply and not review.draft_reply:
        return RedirectResponse(
            url=f"/reviews/{review.id}?message=Нельзя отправить пустой финальный ответ",
            status_code=303,
        )
    ...
    log_action(
        db,
        "review_action",
        f"Для отзыва #{review.id} выполнена отправка ответа. Успех: {sent}.",
    )
    return RedirectResponse(
        url=(
            f"/reviews/{review.id}?message="
            + ("Ответ отправлен" if sent else "Не удалось отправить ответ")
        ),
        status_code=303,
    )
...
def archive_review(review_id: int, db: Session = Depends(get_db)):
    ...
    log_action(
        db,
        "review_action",
        f"Отзыв #{review.id} переведен в архив.",
    )
    return RedirectResponse(
        url=f"/reviews/{review.id}?message=Отзыв отправлен в архив",
        status_code=303,
    )
